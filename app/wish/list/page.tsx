...
export default function Wishlist() {
  ...
  async function loadWishes(email?: string) {
    setLoading(true);
    try {
      const url = email ? `/api/wish?email=${encodeURIComponent(email)}` : "/api/wish";
  ...
  useEffect(() => {
    document.title = "My Created Wishes | Wishey";
    ...
    <div className="w-full min-h-[calc(100vh-5rem)] p-4 md:p-10 max-w-7xl mx-auto flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-6">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            {currentUser?.email ? (
              <>Showing created wish cards for <strong>{currentUser.email}</strong>.</>
            ) : (
        ...
        <div className="flex items-center gap-3">
          <EarnTokenModal userEmail={currentUser?.email} />
          <Button
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
          ...
          <p className="text-muted-foreground text-sm font-semibold">Fetching your created wish cards...</p>
        </div>
      ) : wishes.length === 0 ? (
